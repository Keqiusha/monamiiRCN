...
class zhilianSpider(object):
    def __init__(self, start_page, end_page, city, job, url):
        ...
        # 浏览器驱动对象
        self.browser = webdriver.Chrome(options=chrome_opt , executable_path="../bin/driver/chromedriver.exe")
    ...
    def write_to_sql(self, list1):
        # 创建数据库的连接
        conn = pymysql.connect(host="127.0.0.1", port=3306, user="root", password="root", db="company_spider_db", charset="utf8")
        ...
        for job in list1:
            try:
                sql = 'insert into company_spider_db values (null,"{}","{}","{}","{}","{}","{}","{}","{}")'.format(job["job"], job["salary"], job["experience"], job["education"], job["welfare"], job["company"][:200]+"···", job["people"], job["address"])
                # 开始
                conn.begin()
                # 写入数据库
                cursor.execute(sql)
